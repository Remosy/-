...
from Demo_gym import error, logger
...
class EnvRegistry(object):
    ...
    def make(self, path, **kwargs):
        ...
        if (env.spec.timestep_limit is not None) and not spec.tags.get('vnc'):
            from Demo_gym.wrappers.time_limit import TimeLimit
            env = TimeLimit(env,
    ...
    def spec(self, path):
        if ':' in path:
            ...
            # catch ImportError for python2.7 compatibility
                ...
                raise error.Error('A module ({}) was specified for the environment but was not found, make sure the package is installed with `pip install` before calling `Demo_gym.make()`'.format(mod_name))
        else:
            id = path
